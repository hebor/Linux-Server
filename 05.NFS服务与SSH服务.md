...
[root@nfs data]# cd /data && rsync -az -R --delete ./  --timeout=100 rsync_backup@172.16.1.41::data --passworle=/etc/rsync.passwd >/dev/null 2>&1    # 手动执行测试命令，观察命令是否执行成功
```
...
如果执行sersync2守护进程后，再执行测试命令发现报错，直接修改xml配置文件后，再执行一次测试命令即可，无需重复执行sersync2命令

#### 实现数据平滑迁移

迁移NFS数据到BACKUP服务器，并将后续数据直接指向BACKUP服务器，在这个过程中还需要保证业务不中断

1. 首先NFS的数据全部实时同步到BACKUP，保证业务迁移时不会因为数据差异大产生较大的影响
