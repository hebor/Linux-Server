...
上述都只是OpenStack比较核心的一些组件，但实际上OpenStack还有一个更加复杂的逻辑结构，当你需要去了解和设计OpenStack系统架构时，就必须清楚OpenStack的逻辑结构

![openstack逻辑结构](./image/openstack逻辑结构.png)

作为初学者而言，在openstack的逻辑结构中只需要关注两点

1. 消息队列

   OpenStack由多个组件组合而成，而每个组件内部又包含了多个服务，组件内的各个服务之间都需要通过消息队列进行通信。例如nova组件，在openstack逻辑结构中，nova组件内部包含`nova-scheduler`、`nova-console`、`nova-cert`等服务，但这些服务之间大多并不直接通信，而是通过消息队列`Queue`实现通信

   消息队列的优势在于解耦和分布式，例如将一个组件的多个服务部署在不同的节点上，通常情况下各个组件的API服务会部署在Controller节点上

2. API

   除了Neutron以外，OpenStack的每个组件都有自己的API服务，Neutron同样需要一个API服务，只不过Neuturn的API服务被集成到了`neutron-server`服务中。组件内部的多个服务之间需要通过消息队列通信，而不同的组件之间则需要通过各个组件的API服务进行通信

   组件的API服务会对外监听，API接口收到请求后会对请求的资源做解析，请求解析参数正常后API服务会将请求消息写入`Queue`，这个组件内的其他服务如果和请求的资源相关联，那么该服务自然会从`Queue`中订阅、消费消息，从而做下一步处理，最终的处理结果仍由API接口返回
...
1. 按OpenStack官网手册，给所有节点安装不同版本的OpenStack软件源
...
2. 在Controller节点上安装openstack客户端工具
   ...
   ```bash
   yum install -y python-openstackclient    # openstack客户端工具仅需要在Controller节点上安装即可，openstack的其他节点也可以选择安装
   ```
...
3. 为openstack创建RabbitMQ账户
   ...
   ```bash
   rabbitmqctl add_user openstack hebor    # 创建openstack用户并设置密码
   rabbitmqctl list_users    # 查看RabbitMQ的用户列表，默认创建的openstack账户没有权限
...
2. 配置Memcached监听的接口
   ...
   systemctl enable --now memcached    # 启用Memcached服务
   ```
