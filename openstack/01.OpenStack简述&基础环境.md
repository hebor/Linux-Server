...
| Block Storage | 2    | 4G   | 3    | 100G+100G | CentOS7  | Cinder-Volume服务；通过驱动对接存储。需注意，Cinder的API组件运行在Controller节点上 |
...
1. 按OpenStack官网手册，给所有节点安装不同版本的OpenStack软件源，此处以controller节点为示例
   ...
   [root@controller ~]# reboot    # 更新系统后重启
   ```
...
3. 为openstack创建RabbitMQ账户
   ...
   [root@controller ~]# rabbitmqctl change_password openstack openstack_password    # 修改openstack用户的密码
   ```
