...
1. PHP程序配置管理文件/etc/php.ini，主要调整日志、文件上传、禁止危险函数、关闭版本号显示等
...
# php-fpm运行时间
# 当前池接受的连接数
# 请求等待队列，如果这个值不为0，则需要增加fpm的进程数
# 请求等待队列最高的数量
# 请求队列的长度
# php-fpm空闲的进程数
# php-fpm活跃的进程数
# php-fpm总的进程数
# php-fpm最大活跃的进程数（FPM启动开始计算）
# 限制过进程最大数量的次数，如果数量不为0，则说明php-fpm最大进程数较小，可以适当调整
```
