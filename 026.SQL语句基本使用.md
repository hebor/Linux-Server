...
GROUP BY teacher.tname,course.cname             #先对teacher.tname进行分组，后再以course
.cname进行分组查询
...
例如下列，`SELECT`后的条件列是`user`和`host`，那么这两个条件列也需要作为`GROUP BY`的条件列，但实际只有`user`作为`GROUP BY`的条件列，此时`host`必须通过函数去涵盖，例如`GROUP_CONCAT()`函数，否则提示`sql_mode`冲突错误；此参数在5.7版本的MySQL中自带，5.6和8.0都没有
