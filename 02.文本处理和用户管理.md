...
echo student{01..02} | tr " " "\n" | sed -rn 's#.*#useradd &; echo &:$(tr -cd '[:print:]' < /dev/urandom | head -c 6) | tee passwd.txt | chpasswd#p' | bash    # 批量创建用户，并将随机设置的6位密码保存到指定文件
```
...
3. 当文件权限为`200`时，所属用户的每一次写入都是覆盖写入
4. 当文件权限为`100`时，所属用户仍无法执行文件，需要搭配读权限
