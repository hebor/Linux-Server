...
**RAID 0**：无差错校验的分条RAID，`RAID 0`的性能就是所有硬盘性能的总和，读写都是以分条为单位进行读写，但任意一块盘的损坏都会造成整个RAID数据不可用。RAID 0一般用于缓存服务器上，存放一些不重要的数据
...
- **Physical Volume，PV，物理卷**
  ...
  PV是LVM的最底层单元，Linux系统上的磁盘分区需要先经过pvcreate指令将其转换为PV后，LVM才能对PV进行下一步动作，每个PV都会映射到一个磁盘分区上。PV、物理卷本质上都是指的参与制作LVM的磁盘分区，只不过是LVM对磁盘分区的不同称呼，不是所有的磁盘分区都被称为PV
...
- **Logical Volume，LV，逻辑卷**
  ...
  LVM整体结构的最上层是LV，LV为Linux提供了创建文件系统的分区环境，用户最终使用的就是LV，每一个LV一定是从VG的资源池中提取的硬盘容量。为了便于管理员使用LVM管理系统，LV的设备名称通常指定为`/dev/vgname/lvname`样式，当管理员将LVM挂载到系统上之后，通过`df -h`查看挂载信息时又会看到LV的另一个设备名`/dev/mapper/vgname-lvname`

  实际上LVM真实的设备名是`/dev/dm-n`样式，`/dev/vgname/lvname`、`/dev/mapper/vgname-lvname`都只不过是`/dev/dm-n`的软连接，这些软链接都是由系统自动创建的。例如，本实验环境中系统创建的Swap分区所使用的LV设备名就是`/dev/dm-1`

- **Physical Extent，PE，物理区块**

  PE是LVM的最小存储单元，LVM的所有数据写入都是借助PE实现的，PE对于LVM就类似于Block对于文件系统，调整PE的数量或大小会直接影响到LVM的最大容量，每个LV的大小也与此LV内的PE数量有关
  
  PE的默认大小为4M，通过vgchange命令可以修改VG的PE大小，但修改PE大小也是有限制的，修改的目标PE的大小只能是2的幂次方，例如8M、16M等
...
1. 创建PV
   ...
   [root@localhost ~]# pvdisplay /dev/sdb1    # 查看已经创建的pv列表详细信息
   [root@localhost ~]# pvs    # 查看pv简要信息
   ```
...
2. 创建VG
   ...
   [root@localhost ~]# vgchange -s 1M vg01    # 修改PE大小
   [root@localhost ~]# vgdisplay vg01    # 查看指定VG组的信息，不指定VG组名则查看所有VG列表
   [root@localhost ~]# vgs    # 查看VG简要信息
   ```
...
3. 创建LV
   ...
   [root@localhost ~]# lvs    # 查看LV简要信息
   ```

4. LV的扩容、缩容

   LV的扩容一定是从创建该LV的VG中继续申请更多的空间，不能从其他的VG中申请空间，如果指定的VG容量不够，也可以先对VG执行扩容。在手动增加或减小LV的大小时，LV中的文件系统需要另外再手动修正大小的改变，大多数文件系统都包含了能够重新格式化文件系统的程序，例如用于ext文件系统的resize2fs程序。LV的扩容与缩容动作完全相反且层层递进，扩容需要先增加LVM的容量再调整文件系统的大小，缩容则需要先调整文件系统的大小再减小LVM的容量
   ...
   [root@localhost ~]# lvextend -L +5G /dev/vg01/lv01    # 将LV的容量扩充5G，如果不使用+号，则代表将该LV的容量扩充到5G，如果目标扩容容量比初始容量要小，LVM会产生错误提示
   [root@localhost ~]# resize2fs /dev/vg01/lv01          # resize2fs用于扩展ext文件系统、——xfs_growfs用于扩展xfs文件系统
   ...
   LV的缩容首先需要调整文件系统大小，再调整磁盘容量大小，在缩减文件系统的时候可能会造成数据丢失，因此缩减操作在生产环境中需要慎用。并且不同的文件系统可能不支持LV的缩容操作，例如，xfs文件系统不支持LV的缩容
...
5. LVM的移除
   ...
   在执行VG的删除之前，需要确保所有基于此VG的LV已经全部删除
