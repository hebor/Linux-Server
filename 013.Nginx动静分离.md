...
# 3.准备站点环境
mkdir -p /usr/local/nginx/html/ccc/bbb/
echo "ccc_bbb" > /usr/local/nginx/html/ccc/bbb/index.html
...
```shell
# 1.准备访问路径
mkdir /usr/local/nginx/html/2020/ccc/bbb/ -p
echo "2020_ccc_bbb" > /usr/local/nginx/html/2020/ccc/bbb/2.html
...
```shell
# 1.站点目录
mkdir /usr/local/nginx/html/course/11/22/33/ -p
mkdir /usr/local/nginx/html/course/44/55/66/ -p
echo "123" > /usr/local/nginx/html/course/11/22/33/course_33.html
echo "456" > /usr/local/nginx/html/course/44/55/66/course_66.html

# 2.nginx配置
location ^/course {
	rewrite (.*)-(.*)-(.*)-(.*)\.(.*) /$1/$2/$3/$4/$1_$4 break;
}
systemctl restart nginx
