...
在早起的Linux上，/etc/passwd文件中有加密后的用户密码，但鉴于很多程序都需要访问/etc/passwd文件获取用户信息，这成为了一个安全隐患，现在绝大多数Linux系统都将用户密码单独存放在/etc/shadow文件中，只有特定程序才能访问这个文件，而/etc/passwd文件中的密码栏则改为了`x`
